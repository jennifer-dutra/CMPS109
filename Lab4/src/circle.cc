...
bool Circle::containedWithin(RegularConvexPolygon &polygon)
{
    ...
    for (Line const &edge: polygon.edges()) {
      if (Geom::intersects(edge, line, true)) {
        return false;
      }
    }
    return true;
...
bool Circle::containedWithin(ReuleauxTriangle &rt) {
  ...
  // check: is circle is in RT
  std::array<Point2D, 3> RTV = rt.vertices();
  // get radii of circle centered at each vertex
  double RTradius = Geom::distance(RTV[0], RTV[1]);

  // if circle is not within circles created from edges of RT, not contained
  for(Point2D const &vertex: rt.vertices()) {
    if( Geom::distance(center(), vertex) > (RTradius - radius()) ) {
      return false;
    }
  }
