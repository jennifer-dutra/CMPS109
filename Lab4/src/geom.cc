...
bool Geom::intersects(const Line &a, const Line &b, bool outsideCheck)
{
    // FIX: if line is the same return false
    ...
    // if all points are colinear, same line
    if(o1 == 0 && o2 == 0 && o3 == 0 && o4 == 0) {
      return false;
    }

    // check if vertex touching but not intersection
...
Point2D Geom::center(ReuleauxTriangle &rt)
{
    double cx = 0.0;
    double cy = 0.0;
    double signedArea = 0.0;
    ...
    for (int i = 0; i < 3; i++)
    {
        x0 = rt.vertices()[i].x;
        y0 = rt.vertices()[i].y;
        x1 = rt.vertices()[(i+1) % 3].x;
        y1 = rt.vertices()[(i+1) % 3].y;
        a = x0*y1 - x1*y0;
        signedArea += a;
        cx += (x0 + x1)*a;
        cy += (y0 + y1)*a;
